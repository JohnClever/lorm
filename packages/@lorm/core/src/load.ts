...
async function findFileWithExtensions(
  basePath: string,
  extensions: readonly string[] = FILE_EXTENSIONS
): Promise<string | null> {
...
async function findProjectFile(type: "router" | "schema"): Promise<string> {
  ...
  if (type === "router") {
    ...
    const legacyPath = await findFileWithExtensions(patterns.legacy, [
      ".js",
      ".ts",
    ]);
    if (legacyPath) {
  ...
  throw new Error(
    ...
    } not found. Expected ${expectedPaths} in project root`
  );
}
...
async function loadTypeScriptModule(modulePath: string): Promise<any> {
  try {
    ...
    try {
      ...
      throw new Error(
        "tsx is required to load TypeScript files. Please install it: npm install --save-dev tsx"
      );
    }
...
`;
    ...
    if (!output.includes("__LORM_MODULE_SUCCESS__")) {
      throw new Error(
        "Failed to load TypeScript module - no success marker found"
      );
    }
    ...
    throw new Error(
      `Failed to load TypeScript module: ${
