...
export class AuditRulesEngine {
  ...
  async auditEnvironmentVariables(projectPath: string): Promise<AuditResult> {
    ...
    Object.keys(process.env).forEach(key => {
      ...
      if (this.isSensitiveVariable(key, value, environment.sensitivePatterns, environment.excludePatterns)) {
      ...
      for (const envFile of envFiles) {
        ...
        if (fs.existsSync(envPath)) {
          try {
            ...
            const secrets = this.scanForHardcodedSecrets(content, environment.sensitivePatterns);
            issues.push(...secrets.map(secret => `Potential hardcoded secret in ${envFile}: ${secret}`));
          } catch (error) {
  ...
  async auditDatabaseConfiguration(projectPath: string): Promise<AuditResult> {
    ...
    if (databaseUrl && database.checkCredentials) {
      ...
      if (database.requireSSL && !databaseUrl.includes('ssl=true') && !databaseUrl.includes('sslmode=require')) {
        issues.push('DATABASE_URL does not enforce SSL connection');
      }
      
      // Check if host is allowed
      const host = this.extractHostFromUrl(databaseUrl);
      if (host && !database.allowedHosts.includes(host) && !this.isLocalHost(host)) {
    ...
    if (database.checkConnectionStrings) {
      const configFiles = ['config/database.yml', 'database.json', 'knexfile.js'];
    ...
    return {
      ...
      details: issues.length > 0 ? issues : ['No database security issues found'],
      fixable: true,
  ...
  async auditFileSystemSecurity(projectPath: string): Promise<AuditResult> {
    ...
    for (const sensitiveFile of filesystem.sensitiveFiles) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          if (filesystem.checkPermissions && this.hasInsecurePermissions(stats)) {
          ...
          if (filesystem.scanForSecrets) {
            ...
            const secrets = this.scanForHardcodedSecrets(content, this.config.auditRules.environment.sensitivePatterns);
            if (secrets.length > 0) {
              issues.push(`Potential secrets found in ${sensitiveFile}: ${secrets.join(', ')}`);
            }
          }
        } catch (error) {
    ...
    if (fs.existsSync(gitIgnorePath)) {
      try {
        ...
        const missingEntries = filesystem.sensitiveFiles.filter(file => 
          !gitIgnoreContent.includes(file) && fs.existsSync(path.join(projectPath, file))
        );
        
        if (missingEntries.length > 0) {
          issues.push(`Sensitive files not in .gitignore: ${missingEntries.join(', ')}`);
        }
      } catch (error) {
    ...
    return {
      ...
      details: issues.length > 0 ? issues : ['No file system security issues found'],
      fixable: true,
      severity: issues.length > 0 ? 'medium' : 'low',
      timestamp: new Date().toISOString()
    };
   ...
   */
  async auditDependencyVulnerabilities(projectPath: string): Promise<AuditResult> {
    const issues: string[] = [];
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (depCount > 100) {
          issues.push(`Large number of dependencies (${depCount}). Consider reducing dependency count.`);
        }
        
        // Check for packages with known security concerns or better alternatives
        const packageRecommendations = {
          'lodash': 'Consider using native ES6+ methods or more specific utility libraries',
          'moment': 'Consider using date-fns, dayjs, or native Date API for better performance',
          'request': 'Package is deprecated, use axios, fetch, or node-fetch instead',
          'node-uuid': 'Package is deprecated, use uuid package instead',
          'colors': 'Consider using chalk or kleur for better performance'
        };
        
        for (const [pkg, recommendation] of Object.entries(packageRecommendations)) {
        ...
        for (const [pkg, version] of Object.entries(allDeps)) {
          if (typeof version === 'string' && (version.startsWith('^0.') || version.startsWith('~0.'))) {
            issues.push(`Package ${pkg} is on major version 0, consider stability`);
          }
        }
    ...
    if (!hasLockFile) {
      issues.push('No lock file found. This can lead to inconsistent dependency versions.');
    }
   ...
   */
  async auditSandboxFunctionality(sandbox: IPluginSandbox): Promise<AuditResult> {
    const issues: string[] = [];
    ...
    try {
      ...
      if (violations.length > 0) {
        issues.push(`${violations.length} security violations detected in sandbox`);
      }
      ...
      const canReadAllowed = sandbox.validatePermissions(auditContext, 'filesystem_read', allowedPath);
      const cannotReadRestricted = sandbox.validatePermissions(auditContext, 'filesystem_read', restrictedPath);
      
      if (!canReadAllowed) {
        issues.push('Sandbox failed to allow permitted filesystem read operation');
      }
      
      if (cannotReadRestricted) {
        issues.push('Sandbox failed to block unpermitted filesystem read operation');
      }
      
      // Test network permission validation
      const canAccessLocalhost = sandbox.validatePermissions(auditContext, 'network_connect', 'localhost:3000');
      const cannotAccessExternal = sandbox.validatePermissions(auditContext, 'network_connect', 'external-site.com:443');
  ...
  async applyFix(result: AuditResult, projectPath: string): Promise<FixResult> {
    ...
    try {
      switch (result.category) {
        ...
        default:
          throw new Error(`Fixes not implemented for category: ${result.category}`);
      }
    } catch (error) {
      return {
  ...
  private createPassResult(category: AuditCategory, message: string): AuditResult {
    return {
      category,
      status: 'pass',
      message,
  ...
  private isSensitiveVariable(key: string, value: string, sensitivePatterns: RegExp[], excludePatterns: RegExp[]): boolean {
    if (!value || value.length === 0) return false;
    
    // Skip if matches exclude patterns
    if (excludePatterns.some(pattern => pattern.test(key) || pattern.test(value))) {
      return false;
    }
    ...
    const isDevelopmentValue = value.includes('localhost') || 
  ...
  private scanForHardcodedSecrets(content: string, patterns: RegExp[]): string[] {
  ...
  private async applyFileSystemFixes(result: AuditResult, projectPath: string, changes: FixChange[]): Promise<FixResult> {
  ...
  private async applyDatabaseFixes(result: AuditResult, projectPath: string, changes: FixChange[]): Promise<FixResult> {
    try {
      ...
      for (const envFile of envFiles) {
        ...
        if (fs.existsSync(envPath)) {
          ...
          const updatedLines = lines.map((line, index) => {
            if (line.includes('DATABASE_URL') && this.hasInsecureCredentials(line)) {
              hasChanges = true;
              changes.push({
                type: 'file_modify',
                target: envFile,
                description: `Added security warning for insecure database URL on line ${index + 1}`,
                oldValue: line,
      ...
      const securityChecklistPath = path.join(projectPath, '.lorm', 'database-security-checklist.md');
      const checklistContent = `# Database Security Checklist\n\n` +
   ...
   */
  private async applyEnvironmentFixes(result: AuditResult, projectPath: string, changes: FixChange[]): Promise<FixResult> {
    try {
      ...
      if (!fs.existsSync(envExamplePath)) {
        ...
        changes.push({
          type: 'file_create',
          target: '.env.example',
          description: 'Created .env.example template with safe placeholder values',
          oldValue: '',
   ...
   */
  private async applyDependencyFixes(result: AuditResult, projectPath: string, changes: FixChange[]): Promise<FixResult> {
    try {
      // Create security audit script
      const auditScriptPath = path.join(projectPath, 'scripts', 'security-audit.js');
      ...
      const auditScriptContent = `#!/usr/bin/env node\n` +
      ...
      if (fs.existsSync(packageJsonPath)) {
        try {
          ...
          if (!packageJson.scripts['security:audit']) {
            packageJson.scripts['security:audit'] = 'node scripts/security-audit.js';
