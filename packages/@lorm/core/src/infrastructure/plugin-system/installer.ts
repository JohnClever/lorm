...
export class PluginInstaller {
  ...
  async installPlugin(options: PluginInstallOptions): Promise<PluginInstallResult> {
    ...
    try {
      ...
      if (!options.force && await this.isPluginInstalled(options.name)) {
        throw new Error(`Plugin '${options.name}' is already installed. Use --force to reinstall.`);
      }
      ...
      return {
        ...
        dependencies: plugin.metadata.dependencies ? Object.keys(plugin.metadata.dependencies) : []
      };
    } catch (error) {
      return {
        success: false,
  ...
  async searchPlugins(query: string, options?: PluginSearchOptions): Promise<PluginSearchResult[]> {
    ...
    const sources = options?.source === 'all' ? ['npm', 'marketplace'] : [options?.source || 'marketplace'];
    ...
    for (const source of sources) {
      try {
        if (source === 'npm') {
          ...
          const marketplaceResults = await this.searchMarketplacePlugins(query, options);
    ...
    if (options?.verified !== undefined) {
      filteredResults = filteredResults.filter(r => r.verified === options.verified);
    }
  ...
  async validatePlugin(plugin: IPlugin): Promise<PluginValidationResult> {
    ...
    if (!plugin.metadata) {
      ...
      if (!plugin.metadata.description) warnings.push('Plugin description is recommended');
      if (!plugin.metadata.author) warnings.push('Plugin author is recommended');
    }
   ...
   */
  private async resolvePluginSource(options: PluginInstallOptions): Promise<PluginSource> {
   ...
   */
  private async resolveNpmPlugin(options: PluginInstallOptions): Promise<PluginSource> {
   ...
   */
  private async resolveLocalPlugin(options: PluginInstallOptions): Promise<PluginSource> {
   ...
   */
  private async resolveGitPlugin(options: PluginInstallOptions): Promise<PluginSource> {
   ...
   */
  private async resolveMarketplacePlugin(options: PluginInstallOptions): Promise<PluginSource> {
    ...
    try {
      ...
      const { stdout } = await execAsync(`npm view ${options.name} version --json`);
      ...
      throw new Error(`Plugin '${options.name}' not found in marketplace: ${error}`);
    }
  }
   ...
   */
  private async downloadPlugin(source: PluginSource, options: PluginInstallOptions): Promise<string> {
   ...
   * Copy local plugin
   */
  private async copyLocalPlugin(source: PluginSource, targetDir: string): Promise<string> {
    try {
   ...
   * Clone Git plugin
   */
  private async cloneGitPlugin(source: PluginSource, targetDir: string): Promise<string> {
  ...
  private async validatePluginSecurity(plugin: IPlugin): Promise<void> {
    ...
    try {
      ...
      const result = await this.loaderContext.sandbox.execute(context, async () => {
      ...
      if (result.violations && result.violations.length > 0) {
        throw new Error(`Security violations detected: ${result.violations.join(', ')}`);
      }
    } catch (error) {
   ...
   */
  private async installDependencies(plugin: IPlugin, pluginPath: string): Promise<void> {
    const packageJsonPath = join(pluginPath, 'package.json');
    ...
    try {
      ...
      console.warn(`Failed to install dependencies for plugin ${plugin.metadata.name}:`, error);
    }
  }
   ...
   * Register plugin in configuration
   */
  private async registerPluginInConfig(plugin: IPlugin, options: PluginInstallOptions): Promise<void> {
    ...
    try {
      ...
      console.log(`Registered plugin ${plugin.metadata.name}@${plugin.metadata.version} in configuration`);
    } catch (error) {
   ...
   * Search NPM plugins
   */
  private async searchNpmPlugins(query: string, options?: PluginSearchOptions): Promise<PluginSearchResult[]> {
    try {
   ...
   */
  private async searchMarketplacePlugins(query: string, options?: PluginSearchOptions): Promise<PluginSearchResult[]> {
    try {
   ...
   */
  private async getInstalledPluginInfo(name: string): Promise<PluginInfo | null> {
    const pluginPath = join(this.pluginsDirectory, name);
   ...
   */
  private async validateInstallRequest(options: PluginInstallOptions): Promise<void> {
    if (!options.name) {
  ...
  private ensureInitialized(): void {
    if (!this.loaderContext || !this.contextFactory) {
      throw new Error('PluginInstaller not initialized. Call initialize() first.');
    }
  }
}