...
import { PluginLifecycleState } from '../types/index.js';
import { createPluginError } from '../utils/validation.js';
 ...
 */
export class PluginLifecycleManagerImpl {
   ...
   */
  getPluginLifecycleManager(pluginName: string): PluginLifecycleManager<any> {
    return {
      getCurrentState: () => this.getState(pluginName),
      getState: () => this.getState(pluginName),
      transition: (to: PluginLifecycleState) => this.transition(pluginName, to),
      onStateChange: (callback: (oldState: PluginLifecycleState, newState: PluginLifecycleState) => void) => this.onStateChange(pluginName, callback),
      executeLifecycleHook: async (hook: keyof PluginLifecycle<any>, ...args: unknown[]) => {
        // Execute lifecycle hook - this would need access to the plugin's lifecycle configuration
        await this.executeTransitionHooks(pluginName, this.getState(pluginName), this.getState(pluginName));
      }
    };
  }
  ...
  private async executeBatchTransition(
    ...
    if (errors.length > 0) {
      const errorMessage = `Batch transition failed for ${errors.length} plugins: ${errors.map(e => e.plugin).join(', ')}`;
      throw createPluginError(
        PluginErrorCode.VALIDATION_ERROR,
        errorMessage,
        { context: { errors } }
  ...
  /**
   * Execute transition hooks for a plugin state change
   */
  private async executeTransitionHooks(
    ...
    // This is a placeholder for the actual hook system integration
    this.eventEmitter.emit(`transition:${pluginName}`, {
      from: fromState,
      to: toState,
      context
    });
  ...
  private notifyStateChange(
    ...
    for (const callback of callbacks) {
      try {
        ...
        console.error(`Error in state change callback for plugin ${pluginName}:`, error);
      }
    }
