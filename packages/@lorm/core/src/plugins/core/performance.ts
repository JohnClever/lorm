...
export class PerformanceManager extends EventEmitter {
  ...
  async preloadCriticalPlugins(
    criticalPlugins: readonly string[] = this.config.criticalPlugins,
    ...
    try {
      ...
      await Promise.all(
        criticalPlugins.map(async (pluginName) => {
          try {
            ...
            console.warn(
              `Failed to preload critical plugin ${pluginName}:`,
              error
            );
          }
        })
      ...
      this.emit("criticalPluginsLoaded", {
        count: criticalPlugins.length,
        loadTime,
      });
    } catch (error) {
  ...
  private async performLoad(
    ...
    try {
      ...
      // Note: Plugin initialization should be handled by the plugin manager
      // with proper context, not by the performance manager
      if (plugin.initialize) {
        const initStartTime = Date.now();
        // TODO: Plugin initialization requires a PluginContext parameter
        // This should be handled by the PluginManager, not PerformanceManager
  ...
  private performMemoryCleanup(): void {
    const entries = Array.from(this.pluginCache.entries()).sort(
  ...
  private getTotalMemoryUsage(): number {
    return Array.from(this.metrics.memoryUsage.values()).reduce(
  ...
  private performPeriodicCleanup(): void {
    ...
    const staleEntries = Array.from(this.pluginCache.entries()).filter(
      ([, entry]) => now - entry.lastAccessed > staleThreshold
    );
