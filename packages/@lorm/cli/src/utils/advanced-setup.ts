...
export async function createSchemaFile(schemaTargetPath: string, rootDir: string): Promise<void> {
  try {
...
export async function createDrizzleConfig(drizzleConfigPath: string, config: any): Promise<void> {
  try {
...
export async function validateSchemaFileOptional(rootDir: string): Promise<void> {
...
export function handleAdvancedCommandError(error: unknown, commandName: string, duration?: number): never {
  const durationText = duration ? ` after ${duration}ms` : "";
  const errorMessage = error instanceof Error ? error.message : String(error);
  
  console.error(`❌ [lorm] ${commandName} failed${durationText}: ${errorMessage}`);
