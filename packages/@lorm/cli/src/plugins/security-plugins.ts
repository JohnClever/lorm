...
export const createSecurityPlugins = async (): Promise<SimplePlugin[]> => {
  const securityAuditPlugin = new SimplePluginBuilder("security-audit", "1.0.0")
    ...
    .addCommand({
      ...
      action: async (args: Record<string, unknown>) => {
        await performSecurityAudit(args as SecurityCommandOptions);
      },
    })
    .addCommand({
      name: "audit",
  ...
  const securityLogsPlugin = new SimplePluginBuilder("security-logs", "1.0.0")
    .description("Security logs plugin")
    .addCommand({
      name: "security:logs",
      ...
      action: async (args: Record<string, unknown>) => {
        await viewSecurityLogs(args as SecurityCommandOptions);
      },
    })
    .addCommand({
      name: "logs",
  ...
  return [securityAuditPlugin, securityLogsPlugin];
};