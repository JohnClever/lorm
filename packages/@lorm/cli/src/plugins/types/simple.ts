...
export function convertSimplePlugin(simplePlugin: SimplePlugin): Plugin {
  const fullPlugin: Plugin = {
    ...
    author: typeof simplePlugin.author === 'string' ? simplePlugin.author : simplePlugin.author?.name || '',
    license: simplePlugin.license ? { type: simplePlugin.license as LicenseType } : { type: 'MIT' as LicenseType },
    
    // Convert simple commands to full commands
    commands: simplePlugin.commands?.map(cmd => ({
      name: cmd.name,
    ...
    hooks: simplePlugin.hooks?.map(hook => ({
      name: hook.name,
    ...
    // Convert permissions
    permissions: simplePlugin.permissions?.map(perm => ({
      name: perm,
 ...
 */
export function createSimplePlugin(name: string, version: string): SimplePluginBuilder {
