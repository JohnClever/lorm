...
/**
 * Executes the drizzle-kit generate command
...
async function executeGenerate(
  ...
  try {
    ...
    await execa(drizzleKitBin, ["generate"], {
      cwd: lormDir,
      stdio: "inherit",
    });

    console.log("✅ [lorm] Migration files generated successfully!");
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(
        `[lorm] Failed to generate migration files: ${error.message}`
      );
    }
    throw new Error(`[lorm] Failed to generate migration files: ${error}`);
  }
}

export async function generate(): Promise<void> {
  try {
    console.log("🔄 [lorm] Starting migration generation process...");
    ...
    await executeGenerate(drizzleKitBin, lormDir);
  } catch (error) {
    console.error("❌ [lorm] Migration generation failed:");
    if (error instanceof Error) {
