...
/**
 * Executes the drizzle-kit studio command
 * @param drizzleKitBin Path to the drizzle-kit binary
 * @param lormDir Directory where drizzle-kit should run
 */
async function executeDrizzleKitStudio(drizzleKitBin: string, lormDir: string): Promise<void> {
  try {
    console.log("🎨 [lorm] Starting Drizzle Studio...");
    console.log("🌐 [lorm] Studio will be available at http://localhost:4983");
    
    await execa(drizzleKitBin, ["studio"], {
      cwd: lormDir,
      stdio: "inherit",
    });
  } catch (error) {
    if (error instanceof Error) {
      if ('exitCode' in error && typeof error.exitCode === 'number') {
        throw new Error(
          `[lorm] drizzle-kit studio failed with exit code ${error.exitCode}: ${error.message}`
        );
      }
      throw new Error(`[lorm] drizzle-kit studio failed: ${error.message}`);
    }
    throw new Error(`[lorm] drizzle-kit studio failed: ${String(error)}`);
  }
}
  ...
  try {
    ...
    if (!jsExists && !tsExists) {
      console.warn(
        ...
        "Studio may not display tables correctly without a schema file."
      );
    }
  } catch (error) {
    console.warn(
      `⚠️ [lorm] Warning: Could not validate schema file existence: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
 ...
 * @param config The loaded lorm configuration
 */
async function validateDatabaseConnection(config: any): Promise<void> {
  try {
    if (!config.database?.url && !config.database?.connectionString) {
      console.warn(
        "⚠️ [lorm] Warning: No database URL found in configuration. " +
        "Studio may not be able to connect to your database."
      );
    } else {
      console.log("🔗 [lorm] Database configuration found");
    }
  } catch (error) {
    console.warn(
      `⚠️ [lorm] Warning: Could not validate database configuration: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function studio(): Promise<void> {
  ...
  try {
    ...
    await executeDrizzleKitStudio(drizzleKitBin, lormDir);

    const duration = Date.now() - startTime;
    console.log(`✅ [lorm] Drizzle Studio started successfully in ${duration}ms`);
  } catch (error) {
    const duration = Date.now() - startTime;
    const errorMessage = error instanceof Error ? error.message : String(error);
    
    console.error(`❌ [lorm] Failed to start Drizzle Studio after ${duration}ms: ${errorMessage}`);
    ...
    if (errorMessage.includes("drizzle-kit not found")) {
      ...
      console.error("💡 [lorm] Port 4983 may be in use. Close other Drizzle Studio instances");
    }
