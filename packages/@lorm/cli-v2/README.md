...
Command-line interface for the **Lorm framework** — a zero-config, full-stack mobile framework built for mobile applications with **type safety**, **security**, and **performance monitoring**.
...
> 🛡️ **Security**: Zero-trust input validation, audit logging, and rate limiting
...
- Watches `lorm/router` for changes
...
Edit `lorm.config.ts` to configure your database:
...
Define your database schema in `lorm/schema/index.ts`:
...
Define your API routes in `lorm/router/index.ts`:
